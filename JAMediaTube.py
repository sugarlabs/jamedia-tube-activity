...
class JAMediaTube(gtk.Window):
    ...
    def eliminarvideos(self, widget):
        ...
        if widget == self.toolbar1:
            ...
            dialog = StandarDialog(self, "Mensaje JAMedia", _("¿ Eliminar Toda la Búsqueda ?"))
            response = dialog.run()
            dialog.destroy()
    ...
    def movervideos(self, widget):
        ...
        if widget == self.toolbar1:
            for child in self.vboxvideos.get_children():
                ...
                tooltips.set_tip(child, _("Arrastra Hacia La Izquierda para Quitarlo de las Descargas"), tip_private=None)
            ...
            for child in self.vboxparadescargar.get_children():
                ...
                tooltips = gtk.Tooltips()
                tooltips.set_tip(child, _("Arrastra Hacia La Derecha para Agregarlo a Descargas"), tip_private=None)
    ...
    def lanzarbusqueda(self, widget, palabras):
        ...
        for video in YT.Buscar(palabras):
            ...
            tooltips = gtk.Tooltips()
            tooltips.set_tip(videowidget, _("Arrastra Hacia La Derecha para Agregarlo a Descargas"), tip_private=None)
    ...
    def set_layout(self):
        ...
        self.vboxparadescargar.drag_dest_set(gtk.DEST_DEFAULT_ALL, targets, gtk.gdk.ACTION_MOVE)
        self.vboxparadescargar.connect("drag-drop", self.drag_drop)
        self.vboxvideos.drag_dest_set(gtk.DEST_DEFAULT_ALL, targets, gtk.gdk.ACTION_MOVE)
    ...
    def drag_drop(self, destino, drag_context, x, y, cosas):
        ...
        if destino == self.vboxparadescargar:
            tooltips = gtk.Tooltips()
            tooltips.set_tip(objeto, _("Arrastra Hacia La Izquierda para Quitarlo de las Descargas"), tip_private=None)
        elif destino == self.vboxvideos:
            tooltips = gtk.Tooltips()
            tooltips.set_tip(objeto, _("Arrastra Hacia La Derecha para Agregarlo a Descargas"), tip_private=None)
